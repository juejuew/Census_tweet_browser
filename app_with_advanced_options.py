...
def parse_data(filename):
    ...
    try:
        if "csv" in filename:
            ...
            df = pd.read_csv(path, encoding = "utf-8", delimiter=r"\s+")
...
def subset_docWordMatrix(button, docWordMatrix_orig_json, removeWords, keepWords):
    if docWordMatrix_orig_json:
        ...
        if len(new_rows) == 0:
            ...
            subset_info_remove = ''; subset_info_keep = 'Meaningless! Remove and keep the same word(s). Please re-enter the words.'
...
def dimRed_and_clustering(docWordMatrix_orig_json, docWordMatrix_json, allMessages_json, new_indices_json,
    ...
    return dimRed_cluster_plot, dimRed_cluster_plot, display_df_columns, display_df_data, unique_cluster_options, unique_cluster_options, json.dumps(cluster_ids_dict), feature_cluster_dot_plot, words_freq_threshold, feature_words
