...
main_layout = html.Div([
    ...
    dbc.Row(dbc.Col(dbc.Spinner(children=[html.Div(id='subset_info_remove'),
                                 html.Div(id='subset_info_keep'),], color="primary", type="border", fullscreen=False,),
    ...
    html.Div([
        ...
        html.Div(id='advanced_options', children=[
            ...
            dcc.Dropdown(id='clustering_method1', multi=False,
                options=[{'label': 'Gaussian Mixture Model', 'value': 'gmm'},
                    ...
                    {'label': 'Leiden community detection', 'value': 'leiden'}],
                value='gmm'),
        ...
        # for leiden: number of nearest neighbors
        html.Div(id='num_neighbor_input_container',
            children=[
                html.H6('Number of nearest neighbors'),
                dcc.Input(id='num_neighbors', type='number', value=1000)
            ]
        )
    ], style=style),
    ...
    html.Div([
        ...
        html.Div(id='manual_select_options', children=[
            ...
            * To accumulate the selection data, please hold down the shift button on keyboard while selecting.
            ...
            dbc.Row(dbc.Col(
            dbc.Spinner(children=[html.Div(id='output_tweet_count')], color="primary", type="border", fullscreen=False,),
            width={'size': 12, 'offset': 0}),
               ),

            # html.Div([
            #     html.P(),
            #     html.Button(id='clear_button1', children='Clear Selected'),
            #     html.P()
            # ], style=style),
    ...
    html.Div(id='group_idx', style={'display': 'none'}),
])
...
def choose_features(cluster_table, top_words_num = 5, threshold = 60):
    ...
    if perc_threshold <= 1:
        perc_threshold = 2
    top_words = []
    topWords_str = 'Top ' + str(top_words_num) + ' Stemmed Words (sort by frequency in decreasing order)'
...
## https://public.paws.wmcloud.org/12410844/100days/100days-Day-026.ipynb
def cluster_polis_leiden(points, num_neighbors):
    A = kneighbors_graph(
        points, 
        num_neighbors, 
        mode="connectivity", 
...
def get_allMessages(isCompleted, fileNames):
    if isCompleted:
        ...
        if isinstance(allMessages, pd.DataFrame):
            ...
            num_tweets = allMessages.shape[0]
            allMessages_json = allMessages.to_json()

            upload_status = ' Finished uploading ：' + fileNames[0] + ' | ' + str(num_tweets) + ' tweets have been uploaded.'
    ...
    if docWordMatrix_orig_json:
        ...
        if removeWords is not None: # if any remove words entered
            if len(removeWords)>0:
                ...
                if len(incorrect_words_ori) > 0:
                    ################################
                    remove_rowSums = [0] * docWordMatrix_update.shape[0]
                    #remove_rowSums = docWordMatrix_update.tocsr().sum(axis=1)
                    ################################
                    subset_info_remove1 = ''
                    subset_info_remove2 = 'Input word(s) to remove not valid: '
                    for i in range(len(incorrect_words_ori)):
                        subset_info_remove2 += incorrect_words_ori[i] + ','
                    subset_info_remove2 = subset_info_remove2[:-1] + '. Please re-enter words!'
        ...
        if keepWords is not None: # if any keeps words entered
            if len(keepWords)>0:
                ...
                if len(incorrect_words_ori) > 0:
                    ##############################
                    # keep_rowSums = docWordMatrix_update.tocsr().sum(axis=1)
                    keep_rowSums = [1] * docWordMatrix_update.shape[0]
                    ##############################
                    subset_info_keep1 = ''
                    subset_info_keep2 = 'Input word(s) to keep not valid: '
                    for i in range(len(incorrect_words_ori)):
                        subset_info_keep2 += incorrect_words_ori[i] + ','
                    subset_info_keep2 = subset_info_keep2[:-1] + '. Please re-enter words!' 
        ...
        if num_new_rows == 0:
            ...
            num_new_rows = len(new_rows)
            subset_info_remove = 'Subset data has ' + str(num_new_rows) + ' tweets in total. '; subset_info_keep = 'Meaningless! Remove and keep the same word(s). Please re-enter the words.'
        else:
            subset_info_keep = subset_info_keep + ' ' + 'Subset data has ' + str(num_new_rows) + ' tweets in total. '
...
@app.callback(Output('clusters_table', 'columns'),
    ...
    Input('group_idx', 'children'))
def generate_top_words_table(cluster_ids_json, allMessages_json, docWordMatrix_json, new_indices_json, 
                             top_words_num, feature_perc_threshold, group_idx):
    if docWordMatrix_json is None:
        display_df = pd.DataFrame(data={'Cluster': [], 'Proportion of Tweets':[],
            'Number of Tweets':[], 'Top 5 Stemmed Words (sort by frequency in decreasing order)':[]})
        ...
        return display_df_columns, display_df_data, unique_cluster_options, {}, None, None
    ...
    for i in range(len(json_ids)-1):
        # get tweet IDs for tweets in the specific cluster
        clusters_data.append(i)
        cluster_ids = json_ids[str(i)]
        cluster_indices = [allMessages_plot.index.get_loc(element) for element in cluster_ids]
        # restrict docWordMatrix to rows in cluster i
        clusteri = docWordMatrix[cluster_indices, :]
        # get most common words
        colSumsi = np.squeeze(np.asarray(clusteri.sum(axis=0)))
        topN = np.sort(colSumsi)[-top_words_num:][::-1]
        topWordsi = []
        for k in range(len(topN)):
            feature = [feature_names[j] for j in range(len(colSumsi)) if colSumsi[j] == topN[k]][0]
            topWordsi.append(feature)
        # add to display data
        ...
        subset_data = allMessages_plot.loc[cluster_ids,:]
    ...
    return display_df_columns, display_df_data, unique_cluster_options, feature_cluster_dot_plot, words_freq_threshold, feature_words
...
def dimRed_and_clustering(docWordMatrix_orig_json, docWordMatrix_json, allMessages_json, new_indices_json,
    ...
    clustering_when, clustering_method1, num_clusters, min_obs, num_neighbors):
    if docWordMatrix_json is None:
        display_df = pd.DataFrame(data={'Cluster': [], 'Proportion of Tweets':[],
            'Number of Tweets':[], 'Top 5 Stemmed Words (sort by frequency in decreasing order)':[]})
        display_df_columns = [{'name': col, 'id': col} for col in display_df.columns]
        display_df_data = display_df.to_dict(orient='records')
        unique_cluster_options = [{'label': '', 'value': ''}]
        return {}, {}, {},unique_cluster_options, None, {}
    # read in document-word matrix
    json_data = json.loads(docWordMatrix_json)
    data = json_data['data_new']
    rows = json_data['rows_new']
    ...
    if clustering_method1 == 'gmm':
        ...
        clusters = cluster_polis_leiden(clustering_data, num_neighbors=num_neighbors)
    # make plot
    allMessages_plot = allMessages.loc[indices]
    allMessages_plot['Cluster'] = clusters # color by cluster
    allMessages_plot['Text'] = allMessages_plot['Message'].apply(lambda t: "<br>".join(textwrap.wrap(t))) # make tweet text display cleanly
    ...
    dimRed_cluster_plot = px.scatter(allMessages_plot, x='coord1', y='coord2', color='Cluster',
        hover_data=['Text', 'MessageId'], category_orders={"Cluster": [str(int) for int in list(range(50))]})
    ...
    for i in set(allMessages_plot['Cluster']):
        ...
        # get most common words
        colSumsi = np.squeeze(np.asarray(clusteri.sum(axis=0)))
        topN = np.sort(colSumsi)[-5]
        topWordsi = [feature_names[j] for j in range(len(colSumsi)) if colSumsi[j]>=topN][0:5]
        # add to display data
        clusters_data.append(i)
        topWords_data.append(" ".join(topWordsi))
        prop_data.append(round(clusteri.shape[0]/docWordMatrix.shape[0], 3))
        ...
        num_unique.append(len(pd.unique(subset_data['Message'])))
        cluster_ids_dict[i] = allMessages_plot[allMessages_plot['Cluster']==i].index.tolist()
    topWords_str = 'Top ' + str(5) + ' Stemmed Words (sort by frequency in decreasing order)'
    display_df = pd.DataFrame(data={'Cluster':[int(x) for x in clusters_data], 'Proportion of Tweets':prop_data,
        'Number of Tweets':num_data, 'Number of Unique Tweets': num_unique, topWords_str:topWords_data})
    display_df_columns = [{'name': col, 'id': col} for col in display_df.columns]
    display_df_data = display_df.to_dict(orient='records')
    ...
    return dimRed_cluster_plot, dimRed_cluster_plot, dimRed_cluster_plot, unique_cluster_options, json.dumps(cluster_ids_dict), group_idx
'''
# Do dimension reduction, clustering
# create plot and table to output
@app.callback(
...
@app.callback(Output('num_clustering_input_container', 'style'),
    ...
    Output('num_neighbor_input_container', 'style'),
    Input('clustering_method1', 'value'))
def cluster_options_formatting(clustering_method):
    if clustering_method in ['gmm', 'k-means']:
        ...
        style_nneighbors={'display':'none'}
    else:
        style_nclusters={'display':'none'}
        if clustering_method in ['hdbscan']:
            style_minObs={'display':'block'}
            style_nneighbors={'display':'none'}
        elif clustering_method in ['leiden']:
            style_nneighbors={'display':'block'}
            style_minObs={'display':'none'}
    return style_nclusters, style_minObs, style_nneighbors
...
def tweet_set_characteristics(unique_cluster_options, cluster_chars, cluster_ids_json, allMessages_json):
    ...
    # make data frames
    if (unique_cluster_options is not None and len(unique_cluster_options) > 0) and (cluster_chars is not None and len(cluster_chars) > 0):
        # get allMessages
        allMessages = pd.read_json(allMessages_json)
        # get tweet ids for tweets in each cluster
        json_ids = json.loads(cluster_ids_json)
        for clust in unique_cluster_options:
            # get tweet IDs for tweets in the specific cluster
            cluster_ids = json_ids[str(clust)]
            ...
            if 'randSamp_follow' in cluster_chars:
                if 'Sender Followers Count' in list(cluster_subset.columns):
                        cluster_subset=cluster_subset.rename(columns = {'Sender Followers Count':'Sender.Followers.Count'})
...
def generate_csv(n_clicks, cluster_ids_json, allMessages_json, download_tweet_cluster):
    ...
    if download_tweet_cluster is not None:
        allMessages = pd.read_json(allMessages_json)
        json_ids = json.loads(cluster_ids_json)
        cluster_ids = json_ids[str(download_tweet_cluster)]
        cluster_subset = allMessages.loc[cluster_ids,:]
...
@app.callback(
        ...
        Input('clear_button1', 'n_clicks'), 
        prevent_initial_call=True)
def clear_selected(n_clicks, selected_subset_json):
        return Non
'''
...
@ app.callback([Output('selected_randomSample_title', 'children'),
    ...
    State('selected_subset_json', 'children')])
def tweet_set_characteristics(selected_chars, selected_subset_json):
    ...
    geo_plot_norm_container = {'display': 'none'}
    # make data frames
    if selected_subset_json is not None and selected_chars is not None:
        # get allMessages
        selected_subset = pd.read_json(selected_subset_json)
        ...
        if 'randSamp_follow' in selected_chars:
            if 'Sender Followers Count' in list(selected_subset.columns):
                        selected_subset=selected_subset.rename(columns = {'Sender Followers Count':'Sender.Followers.Count'})
            randomSampleFollow_title = 'Random Sample of Tweets, Weighted by Follower Count:'
